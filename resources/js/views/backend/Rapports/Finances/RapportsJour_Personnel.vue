                           ...
                           <v-flex xs12 sm12 md12 lg12>
                            ...
                            <v-flex xs12 sm12 md12 lg12>
                                    ...
                                    <v-autocomplete label="Selectionnez le Poste" prepend-inner-icon="mdi-map"
                                        :rules="[(v) => !!v || 'Ce champ est requis']" :items="postList" item-text="nom_poste"
                                        item-value="id" dense outlined v-model="svData.refPoste" chips clearable>
                                    </v-autocomplete>
                                    </div>
                                </v-flex>
                            <br>
                            <v-tooltip bottom color="black">
                                <template v-slot:activator="{ on, attrs }">
                                    <span v-bind="attrs" v-on="on">
                                        <v-btn @click="showPaiementPersonnel_MoisPosteByDate" block color="  blue" dark>
                                            <v-icon>print</v-icon> RAPPORTS DE PAIEMENT DES AGENTS/MOIS/POSTE
                                        </v-btn>
                                    </span>
                                </template>
                                <span>Imprimer le rapport</span>
                            ...
                            <v-tooltip bottom color="black">
                                <template v-slot:activator="{ on, attrs }">
                                    <span v-bind="attrs" v-on="on">
                                        <v-btn @click="showPaiementPersonnel_MoisProjetByDate" block color="  blue" dark>
                                            <v-icon>print</v-icon> RAPPORTS DE PAIEMENT DES AGENTS/MOIS/PROJET
                                        </v-btn>
                                    </span>
                                </template>
                                <span>Imprimer le rapport</span>
...
export default {
    ...
    fetchListPoste() {
      this.editOrFetch(`${this.apiBaseURL}/fetch_dopdown_poste_pers`).then(
        ...
        showPaiementPersonnel_MoisPosteByDate() {
            //refRubrique
            if(this.svData.refMois!="" && this.svData.refAnne!=""&& this.svData.refPoste!="")
                {
                    window.open(`${this.apiBaseURL}/fetch_rapport_paiement_date_mois_poste?refMois=` + this.svData.refMois+"&refAnne="+this.svData.refAnne+"&refPoste="+this.svData.refPoste);
                }else
                {
                    this.showError("Veillez selectionner le mois et l'année svp");
                }  
        },
        showPaiementPersonnel_MoisProjetByDate() {
            //refRubrique
            if(this.svData.refMois!="" && this.svData.refAnne!=""&& this.svData.projet_id!="")
                {
                    window.open(`${this.apiBaseURL}/fetch_rapport_paiement_date_mois_projet?refMois=` + this.svData.refMois+"&refAnne="+this.svData.refAnne+"&projet_id="+this.svData.projet_id);
                }else
                {
                    this.showError("Veillez selectionner le mois et l'année svp");
                }  
